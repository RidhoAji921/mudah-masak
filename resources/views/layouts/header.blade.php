<nav class="bg-orange-300 border-b-[2px] border-black px-4 py-[6px] flex justify-between select-none fixed top-0 left-0 z-10 w-full">
    <div>
        <a href="/">
    ...
    <div class="flex items-center gap-2">
        <div class="flex border-[1px] border-black p-1 rounded-3xl gap-1">
            ...
            <form action="/search-post" method="GET">
                <input type="text" name="query" id="id" placeholder="Cari Resep" value="{{ request('query') }}" class="bg-orange-300 placeholder-black focus:outline-none">
            </form>
        </div>
        @auth
            <p class="text-xl font-medium">Halo! {{ auth()->user()->name }}</p>
            <div class="grid grid-cols-2 gap-1">
                <a class="flex justify-center gap-1 border-[1px] border-black rounded-lg py-1 px-1 hover:bg-orange-400" href="/profile">Profil<svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-user"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M8 7a4 4 0 1 0 8 0a4 4 0 0 0 -8 0" /><path d="M6 21v-2a4 4 0 0 1 4 -4h4a4 4 0 0 1 4 4v2" /></svg></a>
